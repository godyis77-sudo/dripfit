  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-background px-6">
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="relative z-10 w-full max-w-sm">
        {/* Back button */}
        <Button variant="ghost" size="icon" onClick={() => navigate('/')} className="mb-4 h-10 w-10 rounded-xl">
        ...
        <div className="flex flex-col items-center mb-5">
          ...
          <span className="font-display font-bold text-sm tracking-wide text-muted-foreground">DRIP FIT</span>
        </div>

        <Card className="rounded-2xl border-border/30">
          <CardHeader className="text-center pb-2">
            <CardTitle className="font-display text-2xl">{isLogin ? 'Welcome Back' : 'Join DRIP FIT'}</CardTitle>
          ...
          <CardContent className="space-y-4">
            ...
            <div className="flex gap-2">
              <Button
                variant="outline"
                className="flex-1 h-11 rounded-xl text-sm font-medium active:scale-95 transition-transform"
            ...
            <form onSubmit={handleSubmit} className="space-y-3">
              {!isLogin && (
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    ...
                    <Input id="name" placeholder="Your name" value={displayName} onChange={e => setDisplayName(e.target.value)} className="pl-9 rounded-xl h-11" required />
                  </div>
                </div>
              )}
              <div className="space-y-1.5">
                ...
                <div className="relative">
                  ...
                  <Input id="email" type="email" placeholder="you@example.com" value={email} onChange={e => setEmail(e.target.value)} className="pl-9 rounded-xl h-11" required />
                </div>
              </div>
              <div className="space-y-1.5">
              ...
              <Button type="submit" className="w-full h-12 rounded-xl btn-luxury text-primary-foreground font-display font-bold text-base uppercase tracking-wider" disabled={loading}>
            ...
            <p className="text-[10px] text-muted-foreground text-center flex items-center justify-center gap-1 pt-1">
              <Shield className="h-3 w-3" /> Photos are private by default and can be deleted anytime.
            </p>
          </CardContent>
        </Card>
      </motion.div>
