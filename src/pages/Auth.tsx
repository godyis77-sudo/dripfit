  ...
  return (
    <div className="flex min-h-screen flex-col items-center justify-center bg-background px-5">
      ...
      <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} className="relative z-10 w-full max-w-[320px]">
        <Button variant="ghost" size="icon" onClick={() => navigate('/')} className="mb-3 h-8 w-8 rounded-lg">
        ...
        <Card className="rounded-xl border-border/40">
          ...
          <CardContent className="space-y-3 px-4 pb-4">
            <div className="flex gap-2">
              <Button variant="outline" className="flex-1 h-10 rounded-lg text-[12px] font-medium active:scale-[0.97] transition-transform" onClick={() => handleSocialLogin('google')} disabled={!!socialLoading}>
              ...
              <Button variant="outline" className="flex-1 h-10 rounded-lg text-[12px] font-medium active:scale-[0.97] transition-transform" onClick={() => handleSocialLogin('apple')} disabled={!!socialLoading}>
            ...
            <div className="relative">
              ...
              <div className="relative flex justify-center text-[11px]"><span className="bg-card px-2 text-muted-foreground">or continue with email</span></div>
            </div>

            <form onSubmit={handleSubmit} className="space-y-2.5">
              {!isLogin && (
                <div className="space-y-1">
                  <Label htmlFor="name" className="text-[11px]">Display Name</Label>
                  <div className="relative">
                    <User className="absolute left-2.5 top-2.5 h-3.5 w-3.5 text-muted-foreground" />
                    <Input id="name" placeholder="Your name" value={displayName} onChange={e => setDisplayName(e.target.value)} className="pl-8 rounded-lg h-9 text-[13px]" required />
                  </div>
                </div>
              )}
              <div className="space-y-1">
                ...
                <div className="relative">
                  ...
                  <Input id="email" type="email" placeholder="you@example.com" value={email} onChange={e => setEmail(e.target.value)} className="pl-8 rounded-lg h-9 text-[13px]" required />
                </div>
              </div>
              <div className="space-y-1">
                ...
                <div className="relative">
                  ...
                  <Input id="password" type={showPassword ? 'text' : 'password'} placeholder="••••••••" value={password} onChange={e => setPassword(e.target.value)} className="pl-8 pr-9 rounded-lg h-9 text-[13px]" minLength={6} required />
                  <button type="button" onClick={() => setShowPassword(!showPassword)} className="absolute right-2.5 top-2.5 text-muted-foreground hover:text-foreground transition-colors">
                    {showPassword ? <EyeOff className="h-3.5 w-3.5" /> : <Eye className="h-3.5 w-3.5" />}
                  </button>
                </div>
              </div>
              {isLogin && <button type="button" onClick={handleForgotPassword} className="text-[11px] text-primary hover:underline">Forgot password?</button>}
