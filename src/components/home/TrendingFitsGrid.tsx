        ...
        <div className="grid grid-cols-3 gap-2">
          {fits.slice(0, 6).map((fit) => (
            ...
            >
              ...
              <div className="p-2">
                <p className="type-data text-[10px] tracking-widest uppercase text-zinc-500">{fit.brand}</p>
                <p className="font-sans text-sm text-zinc-200 line-clamp-1 leading-tight">{fit.name}</p>
                {fit.description && (
                  <p className="text-[10px] text-zinc-500 line-clamp-1 leading-snug mt-0.5">{fit.description}</p>
                )}
                {fit.price_cents != null && (
                  <p className="type-data text-sm text-foreground font-medium mt-0.5">${(fit.price_cents / 100).toFixed(0)}</p>
                )}
              </div>
            </button>
          ))}
