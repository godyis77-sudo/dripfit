...
const ResultActions = ({ saved, scanDate, onSave, onTryOn, onNewScan, onDelete }: ResultActionsProps) => (
  <div className="space-y-2 mt-3">
    {!saved && (
      <Button className="w-full h-10 rounded-lg text-sm font-bold btn-luxury text-primary-foreground" onClick={onSave}>
    ...
    <Button variant={saved ? 'default' : 'outline'} className={`w-full h-10 rounded-lg text-sm font-bold ${saved ? 'btn-luxury text-primary-foreground' : ''}`} onClick={onTryOn}>
      <Shirt className="mr-1.5 h-4 w-4" /> Try-On with This Profile
    </Button>
