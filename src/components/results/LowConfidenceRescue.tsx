      ...
      {mode === 'waist' && (
        <div className="space-y-1.5">
          <div className="flex gap-1.5">
            ...
            <Button size="sm" className="h-8 rounded-lg px-3 gradient-drip text-primary-foreground text-[11px]" onClick={() => { const v = parseFloat(waistValue); if (v > 0) onCalibrate({ type: 'waist', value: v }); }}>Apply</Button>
          </div>
      ...
      {mode === 'brand' && (
        <div className="space-y-1.5">
          ...
          <div className="flex items-center gap-1.5">
            <Button size="sm" className="h-8 rounded-lg px-3 gradient-drip text-primary-foreground text-[11px]" disabled={!selectedBrand || !selectedSize} onClick={() => { if (selectedBrand && selectedSize) onCalibrate({ type: 'brand', brand: selectedBrand, size: selectedSize }); }}>Apply</Button>
            <button onClick={() => setMode('pick')} className="text-[10px] text-muted-foreground underline">Back</button>
          </div>
        </div>
      )}
