        ...
        <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: 'auto', opacity: 1 }} className="grid grid-cols-2 gap-1.5">
          {keys.map((key, i) => (
            <motion.div key={key} initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }} className="bg-card border border-border rounded-lg px-2.5 py-2">
              ...
              <p className="text-[13px] font-bold text-foreground">{fmtIn(measurements[key])}<span className="text-[11px] font-normal text-muted-foreground ml-0.5">in</span></p>
              <p className="text-[11px] text-muted-foreground">{fmtCm(measurements[key])}<span className="text-[11px] ml-0.5">cm</span></p>
            </motion.div>
          ))}
