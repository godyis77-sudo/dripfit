...
const MOCK_LINES = [
  { key: 'shoulder', label: 'Shoulder', side: 'right' as const, x1: 37.7, y1: 20.2, x2: 62.3, y2: 20.2, labelTop: '18.5%', leaderFromX: 62.3 },
  { key: 'chest', label: 'Chest', side: 'left' as const, x1: 41.2, y1: 25.9, x2: 58.8, y2: 25.9, labelTop: '24%', leaderFromX: 41.2 },
  { key: 'waist', label: 'Waist', side: 'right' as const, x1: 43, y1: 39.5, x2: 57, y2: 39.5, labelTop: '38%', leaderFromX: 57 },
  { key: 'hips', label: 'Hips', side: 'right' as const, x1: 40.4, y1: 47.4, x2: 59.6, y2: 47.4, labelTop: '46%', leaderFromX: 59.6 },
  { key: 'inseam', label: 'Inseam', side: 'left' as const, x1: 47.4, y1: 50.4, x2: 44.7, y2: 90, labelTop: '63%', leaderFromX: 45.6 },
];
...
const DecorativeSilhouette = ({ className = '', height = 200 }: Props) => {
  const imgWidth = height * (2 / 3);
  const pad = 40; // horizontal padding for labels
  const totalWidth = imgWidth + pad * 2;

  return (
    <div className={`relative ${className}`} style={{ width: totalWidth, height }}>
      {/* Centered image + SVG overlay area */}
      <div className="absolute" style={{ left: pad, top: 0, width: imgWidth, height }}>
        <img
          src={bodySilhouette}
      ...
      {/* Labels — positioned relative to the full container so they sit in the padding area */}
